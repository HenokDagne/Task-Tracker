...
if (!token) {
    // Create a full-page overlay for login with a single button
    ...
    loginOverlay.innerHTML = `
      <div style="background: white; padding: 2rem; border-radius: 1rem; box-shadow: 0 2px 16px rgba(0,0,0,0.2); min-width: 320px; max-width: 90vw; text-align: center;">
        <h2 style="font-size: 2rem; margin-bottom: 1rem; color: #4F46E5;">Login Required</h2>
        <button id="loginRedirectBtn" style="background: #4F46E5; color: white; padding: 0.75rem 2rem; border-radius: 0.5rem; font-weight: bold; border: none; font-size: 1.2rem;">Login</button>
      </div>
    `;
    ...
    function Todolist() {
        ...
        // Render tasks after DOM is ready
          fetchTasks().then(tasks => {
            if (Array.isArray(tasks)) {
                tasks.forEach(taskObj => {
                    taskRender(
                        taskObj.title,
                        taskObj.category && taskObj.category.name ? taskObj.category.name : (taskObj.category || ""),
                        taskObj.progress,
                        taskObj.Due_Date
                    );
                });
            } else {
                console.error('fetchTasks did not return an array:', tasks);
            }
        });
